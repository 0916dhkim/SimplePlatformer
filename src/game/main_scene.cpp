...
void MainScene::Play(Stage &stage) const {
  ...
  if (a1 != nullptr) {
    a1->SetRenderer(new TriangleRenderer(a1->GetTransform(), 1, {255, 255, 255, 255}));
    a1->SetBody(new DynamicTriangle(a1->GetTransform(), 1));
    a1->SetPosition(3, 8);
    a1->SetPivot(0.5, 0.5);
  }
  ...
  if (a2 != nullptr) {
    a2->SetPosition(0, 13);
    a2->SetPivot(0, 0.5);
    a2->SetRenderer(new TriangleRenderer(a2->GetTransform(), 5, {0, 255, 0, 255}));
    a2->SetBody(new DynamicTriangle(a2->GetTransform(), 5));
  }

  std::shared_ptr<Actor> a3 = stage.AddActor(20, 0);
  if (a3 != nullptr) {
    a3->SetRotation(0.1);
    a3->SetRenderer(new LineRenderer(a3->GetTransform(), 20, {255, 0, 0, 255}));
    a3->SetBody(new StaticLine(a3->GetTransform(), 20));
    a3->SetPosition(-7, -7);
  }
}
